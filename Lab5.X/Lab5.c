...
{     
   ...
   adLow = ADRESL;   //assign variables for ADC output
   adHigh = ADRESH;
        
   adHigh = adHigh << 8;    //Get last 2 bits from adHigh
        
   decimal = adLow + adHigh;    //Produce 10bit binary
        
   voltage = (decimal*98)/1023+2;   //2-100
   voltage_int = voltage;           //assign as internal pot
   ...
   adHigh_5k = adHigh_5k << 8;      //similar procedure as before but with external pot
        
   decimal_5k = adLow_5k + adHigh_5k;      
        
   voltage_5k = (decimal_5k*50)/1023;   
   voltage_ext = voltage_5k;
   ...
   ADCON1 = 0b10001110;
   Print_To_LCD(Str_1);
   voltage_ext = voltage_ext/10;    //0-5
    
   for (time = 0; time<1; time = time + 0.001) //Create time for sin equation
   {
      PORTCbits.RC0 = 0;        //Turn on for DAC
      PORTCbits.RC5 = 1;
      ...
      //Scale Sin Wave
      dataByte = ((dataByte+5)/10)*255;
     
      SSPBUF = dataByte;
      while(SSPSTATbits.BF == 0);
      
      PORTCbits.RC0 = 1;        //Turn off DAC
      PORTCbits.RC5 = 0;
   }
